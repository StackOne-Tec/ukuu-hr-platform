...
import { getDevices, getClockEvents, getPendingBridgeEvents } from "@/lib/queries";
...
export default async function ImportAttendancePage() {
  const [devRes, evRes, pendingRes] = await Promise.all([
    getDevices(),
    getClockEvents(),
  ...
  return <ImportAttendance devices={devices} recentEvents={recentEvents} pendingEvents={pendingRes.data} />;
}